...
class VoluumApi extends AppModel {
	...
	private function _fetchAuthToken() {
		...
		// ƒл€ auth-токена свой собственный кэш
	...
	private function _writeTokenCache($response) {
		...
		$response['expirationTimestamp'] = date('Y-m-d H:i:s', time() + 30 * MINUTE); // str_replace('T', ' ', $expired); почему-то авторизаци€ летает раньше при указанной даты
	...
	private function _parseDatetime($time) {
		...
		// хот€ принимает даты в обычном Y-m-d формате
	...
	public function getTrackerCampaignList() {
		...
		// суд€ по моим тестам список кампаний не зависит от передаваемой даты
	...
	public function getCampaignDetailedList($campaignURL) {
		//  ака€-то начальна€ дата, чтобы выгрести все данные
		// „ем более ранн€€ дата - тем дольше выполн€етс€ запрос
		$from = $this->_parseDatetime(Configure::read('date.from'));
		$to = $this->_parseDatetime(Configure::read('date.to')); // т.к. часы скидываютс€, нужно брать на день вперед
		...
		foreach($response['rows'] as $row) {
			// »гнорить ID кампаний типа "пусто" или "{campaign_id}"
	...
	public function getDomainList($campaignId, $srcCampaignId = null) {
		 ...
		 */
		$from = $this->_parseDatetime(Configure::read('date.from'));
		$now = $this->_parseDatetime(Configure::read('date.to')); // т.к. часы скидываютс€, нужно брать на день вперед
